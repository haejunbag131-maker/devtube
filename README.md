...
| 문제                                      | 개선 방향                                    | 결과                                                  |
| ----------------------------------------- | -------------------------------------------- | ----------------------------------------------------- |
| API 요청 상태가 컴포넌트마다 분산됨       | React Query 기반 서버 상태 관리로 전환       | 캐싱, 중복 요청 방지, 로딩·에러 상태 관리 일원화      |
| 모바일에서 YouTube 컨트롤 조작이 불안정함 | ReactPlayer 제거 후 YouTube iframe 직접 사용 | iPhone SE 크기에서도 재생·일시정지·컨트롤 조작 안정화 |
| API 응답 전 빈 화면이 노출됨              | 실제 화면 구조와 유사한 스켈레톤 UI 적용     | 검색·채널·영상 상세 페이지의 로딩 UX 개선             |
| 홈 화면에서 외부 이미지 요청이 많음       | 하단 슬라이더 지연 마운트, 썸네일 경량화     | 초기 네트워크 요청과 이미지 전송량 감소               |
| 사용자에게 API 실패 원인이 드러나지 않음  | 공통 에러 메시지와 429 전용 안내 문구 추가   | 요청 한도 초과 상황을 사용자에게 명확히 안내          |
...
실제 사용자 기준에서 기본 접근성과 검색 공유 품질을 보강했습니다.
...
홈 화면은 추천 개발자와 여러 영상 슬라이더를 함께 보여주기 때문에, 초기 진입 시 외부 YouTube 썸네일 요청이 많아질 수 있었습니다.

해결 방법:

- 첫 화면 주요 이미지는 `eager`와 `fetchpriority="high"`를 유지
- LCP 후보 이미지는 fade-in 애니메이션에서 제외
- 하단 영상 슬라이더는 화면 근처에 진입할 때만 마운트
- 슬라이더 썸네일은 `hq720` 대신 `mqdefault`를 사용해 전송량 감소
